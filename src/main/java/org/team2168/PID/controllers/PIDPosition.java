...
public class PIDPosition implements TCPMessageInterface {
	 ...
	 */
	public PIDPosition(String name, double P, double I, double D, PIDSensorInterface currentPos, long period, PowerDistribution pdp) {
	...
	public PIDPosition(String name, double P, double I, double D, double N, PIDSensorInterface currentPos, long period, PowerDistribution pdp) {

		this(name, P, I, D, currentPos, period, pdp);
		this.n = N;
	...
	public PIDPosition(String name, double pUp, double iUp, double dUp, double pDown, double iDown, double dDown,
			PIDSensorInterface currentPos, long period, PowerDistribution pdp) {
		this(name, pUp, iUp, dUp, currentPos, period, pdp);
		this.pGain2 = pDown;
		this.iGain2 = iDown;
		this.dGain2 = dDown;
		...
		if (enable) {
			...
			if (Math.abs(err) <= acceptErrorDiff) {
				if(this.enableHoldingVoltage)
					co = this.holdingVoltage/pdp.getBatteryVoltage();
				else
					co = 0;
